...
class Articles():
    ...
    def get_url(self, ticker):
        if "{}.PA".format(ticker) in sf.tickers_cac().keys():
            url = "https://www.boursorama.com/cours/actualites/1rP{}/".format(ticker)
        else:
            url = "https://www.boursorama.com/cours/actualites/{}/".format(ticker)
        self.response = requests.get(url, headers=self.headers)
        self.soup()

    def soup(self):
        soup = BeautifulSoup(self.response.text, 'html.parser')
        articles_html = soup.find('article').findAll('li', {'class': 'c-list-details-news__line / o-flag'})
        ...
        for index, i in enumerate(articles_html):
            ...
            descritpion = i.find('p', {'class': 'c-list-details-news__content'}).string.strip()
            link = i.find('a')['href']

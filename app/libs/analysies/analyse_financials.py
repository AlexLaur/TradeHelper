...
class AnalyseFondamental(object):
    ...
    def set_var(self):
        ...
        if self.statistic_datas["Value"][9][-1] == "B":
            self.actions = (
                float(self.statistic_datas["Value"][9].replace("B", ""))
                * 1000000000
            )
        if self.statistic_datas["Value"][9][-1] == "M":
            self.actions = (
                float(self.statistic_datas["Value"][9].replace("M", ""))
                * 1000000
            )
        ...
        self.chiffre_affaire = self.resultat_datas.loc[
            "grossProfit"
        ]  # benefice brut
        ...
        self.actifs_total_short_therm = self.balance_datas.loc[
            "totalCurrentAssets"
        ]
        self.passif_total_short_therm = self.balance_datas.loc[
            "totalCurrentLiabilities"
        ]
        self.total_capitaux_propre = self.balance_datas.loc[
            "totalStockholderEquity"
        ]

        self.free_cash_flow = self.cash_flow_datas.loc[
    ...
    def profitability(self):
        ...
        avg_assets = (
            self.actifs_total[self.year_atual]
            + self.actifs_total[self.year_before]
        ) / 2
        avg_assets_py = (
            self.actifs_total[self.year_before]
            + self.actifs_total[self.resultat_datas.keys()[2]]
        ) / 2
        ...
        profitability_score = (
            ni_score + ni_score_2 + op_cf_score + roa_score + ac_score
        )
    ...
    def datas_dict(self):
        self.datas["YEAR"] = [
            date.strftime("%Y") for date in self.resultat_datas.keys().tolist()
        ]
    ...
    def get_histoty_prices(self, tick):
        ...
        for date in dates:
            ...
            try:
                ...
                date_ = (date + datetime.timedelta(days=3)).strftime(
                    "%Y-%m-%d"
                )
                self.price_dates.append(self.history[date_])
    ...
    def bna_years(self):
        bna = [
            round(float(resultat / int(self.actions)), 2)
            for resultat in self.benefice_net.values.tolist()
        ]
        self.datas["BNA"] = bna
    ...
    def dividendes_ratio(self):
        ...
        dividendes = [
            list(set(self.dividendes.loc[year]["dividend"].tolist()))
            for year in self.datas["YEAR"]
        ]
        rendement = []
    ...
    def roe_roa_ratio(self, roa=False):
        ...
        #
        capitaux = [
            int(i.replace(",", "")) for i in self.datas["Capitaux Propre"]
        ]

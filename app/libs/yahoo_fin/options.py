...
def get_options_chain(ticker, date=None, raw=True):
    ...
    if not raw:
        ...
        calls["% Change"] = calls["% Change"].map(
            lambda num: num / 100 if isinstance(num, float) else 0
        )
        calls["Volume"] = (
            calls["Volume"].str.replace("-", "0").map(force_float)
        )
        calls["Open Interest"] = (
            calls["Open Interest"].str.replace("-", "0").map(force_float)
        )

        puts["% Change"] = puts["% Change"].str.strip("%").map(force_float)
        puts["% Change"] = puts["% Change"].map(
            lambda num: num / 100 if isinstance(num, float) else 0
        )
        puts["Volume"] = puts["Volume"].str.replace("-", "0").map(force_float)
        puts["Open Interest"] = (
            puts["Open Interest"].str.replace("-", "0").map(force_float)
        )

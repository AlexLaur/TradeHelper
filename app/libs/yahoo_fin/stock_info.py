...
def get_data(
    ticker, start_date=None, end_date=None, index_as_date=True, interval="1d"
):
    ...
    if interval != "1m":
        ...
        # add in adjclose
        frame["adjclose"] = data["chart"]["result"][0]["indicators"][
            "adjclose"
        ][0]["adjclose"]
...
def tickers_nasdaq(include_company_data=False):
    ...
    index = [splits.index(x) + 1 for x in splits if "\r\n" in x]
    tickers = [
...
def tickers_cac(include_company_data=False):
    """Downloads list of tickers currently listed in the S&P 500 """
    # get list of all S&P 500 stocks
...
def get_quote_table(ticker, dict_result=True):
    ...
    quote_price = pd.DataFrame(
        ["Quote Price", get_live_price(ticker)]
    ).transpose()
...
def get_stats_valuation(ticker):
    ...
    tables = [
        table
        for table in tables
        if "Trailing P/E" in table.iloc[:, 0].tolist()
    ]
...
def get_balance_sheet(ticker, yearly=True):
    ...
    if yearly:
        ...
        temp = json_info["balanceSheetHistoryQuarterly"][
...
def get_cash_flow(ticker, yearly=True):
    ...
    if yearly:
        ...
        temp = json_info["cashflowStatementHistoryQuarterly"][
            "cashflowStatements"
        ]
...
def get_holders(ticker):
    ...
    holders_site = (
        "https://finance.yahoo.com/quote/" + ticker + "/holders?p=" + ticker
    )
...
def get_day_gainers():
    ...
    return _raw_get_daily_info(
        "https://finance.yahoo.com/gainers?offset=0&count=100"
    )
...
def get_next_earnings_date(ticker):
    ...
    temp = parsed_result["context"]["dispatcher"]["stores"][
        "QuoteSummaryStore"
...
def get_earnings_history(ticker):
    ...
    return result["context"]["dispatcher"]["stores"]["ScreenerResultsStore"][
        "results"
    ]["rows"]
...
def get_earnings_in_date_range(start_date, end_date):
    ...
    dates = [
        current_date + datetime.timedelta(diff)
        for diff in range(days_diff + 1)
    ]
